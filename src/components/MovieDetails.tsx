...
import SimilarMovies from "../MovieCategories.tsx/SimilarMovies";


const MovieDetails = () => {
  const { id } = useParams();
  const [movieDetails, setMovieDetails] = useState<MovieItemType>({});
  ...
  useEffect(() => {
    const getMovieDetails = async () => {
      const response = await moviesApiRequest.get(`/movie/${id}`);
      setMovieDetails(response.data);
    };

    const getCasts = async () => {
      const response = await moviesApiRequest.get(`/movie/${id}/credits`);
      ...
      >
        ...
        <div className="lg:flex-3 flex flex-col gap-4 min-h-[800px]">
          <div>
            ...
            <p className="text-sm text-gray-500">{movieDetails?.tagline}</p>
          </div>
          <span className="border-b-[4px] border-b-red-600 w-fit  pb-1">
            Description
          </span>
          <p className="lg:w-[600px] text-gray-500">{movieDetails?.overview}</p>
          <p className="text-sm">
            Votes:
            <span className="text-gray-500"> {movieDetails?.vote_count}</span>
          </p>
          <p className="text-sm">
            Release date:
            <span className="text-gray-500"> {movieDetails?.release_date}</span>
          </p>
          <p className="text-sm">
            Status:
            <span className="text-gray-500"> {movieDetails?.status}</span>
          </p>
          ...
          <p className="text-sm">
            Duration:
            <span className="text-gray-500"> {movieDetails.runtime} mins</span>
          </p>
